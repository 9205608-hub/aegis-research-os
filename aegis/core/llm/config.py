...
class UsageRecord:
    ...
    def estimated_cost_usd(self) -> float:
        ...
        # xAI Grok — published grok-4 rates ($3 in / $15 out per M token).
        # Approximate: cheaper tiers (grok-4-fast etc.) will overestimate;
        # refine per-tier once a live key confirms the served model ids.
        if "grok" in m:
            return (self.input_tokens * 3 + self.output_tokens * 15) / 1_000_000
...
class CostTracker:
    ...
    def summary(self) -> dict:
        ...
        # model — otherwise this clutters non-reasoning-model summaries.
