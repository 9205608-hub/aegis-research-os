...
class VariantAnalyst(AgentBase):
    ...
    def _extract_observations(self, inp: AgentInput) -> list[Observation]:
        ...
        if priced_in:
            ...
            if rev_sig and rev_sig.get("momentum"):
                ...
                observations.append(Observation(
                    text=f"Consensus revision momentum: {priced_in['revision_momentum']}",
                    source_ids=["consensus:revision_momentum"],
                ))

            if priced_in.get("pe_ratio_fwd") is not None:
                observations.append(Observation(
                    text=f"Forward P/E: {priced_in['pe_ratio_fwd']:.1f}x",
        ...
        for name in ("bear", "base", "bull"):
            ...
            if val is not None:
                observations.append(Observation(
                    text=f"{name.capitalize()} case value: {val:.2f}",
        ...
        for d in disagreements:
            if isinstance(d, dict) and d.get("this_is_the_variant"):
                observations.append(Observation(
                    text=f"Variant assumption: '{d.get('assumption', '')}' — "
    ...
    def _derive_inferences(
        ...
        variant_obs = [i for i, o in enumerate(observations) if "variant assumption" in o.text.lower()]
        if variant_obs:
            inferences.append(Inference(
                text="Variant identified: our view differs from market-implied assumptions "
        ...
        price_obs = [i for i, o in enumerate(observations) if "current market price" in o.text.lower()]
        scenario_obs = [i for i, o in enumerate(observations) if "case value" in o.text.lower()]
        if price_obs and scenario_obs:
            ...
            if price > 0 and base > 0:
                ...
                if upside > 0.15:
                    inferences.append(Inference(
                        text=f"Base case implies {upside:.0%} upside — "
                    ...
                    inferences.append(Inference(
                        text=f"Base case implies {upside:.0%} downside — "
        ...
        momentum_obs = [i for i, o in enumerate(observations) if "revision momentum" in o.text.lower()]
        if rev_sig and rev_sig.get("momentum") and momentum_obs:
            ...
            if acceleration == "accelerating":
                txt += ". Revision pace is ACCELERATING — momentum signal strengthening"
            elif acceleration == "decelerating":
                txt += ". Revision pace is DECELERATING — momentum may be fading"
        ...
        if observations:
            inferences.append(Inference(
                text="Edge assessment required: must classify whether edge is analytical, "
        ...
        if sensitivity and disagreements and price_obs:
            decomp_lines = ["Variant decomposition (ΔV = ΔV_growth + ΔV_margin + ΔV_reinvestment):"]
            for d in disagreements:
                if isinstance(d, dict):
                    ...
                    for s in sensitivity:
                        if s.get("assumption", "") in assumption.lower().replace("_fy26", ""):
                            ...
                            impact_str = f" (sensitivity: {impact_pct:.1%} per 10% shock)"
                            break
                    decomp_lines.append(
                        f"  - {assumption}: market={market_val}, ours={my_val}{impact_str}"
                    )
    ...
    def _generate_counterarguments(
        ...
        for inf in inferences:
            if "variant identified" in inf.text.lower():
                counterargs.append(Counterargument(
                    text="Market may already be partially pricing this view through informed "
                         "trading — check positioning and flow data",
                    strength="moderate",
                    evidence_ids=[],
                ))
            elif "upside" in inf.text.lower():
                counterargs.append(Counterargument(
                    text="Apparent undervaluation may reflect risks we underestimate — "
        ...
        counterargs.append(Counterargument(
            text="Variant thesis without near-term catalyst may fail to converge within horizon — "
    ...
    def _identify_disconfirming_triggers(
        ...
        triggers = [
            DisconfirmingTrigger(
                text="Consensus converges to our variant view (edge absorbed by market)",
                monitorable=True,
                check_frequency="monthly",
            ),
            DisconfirmingTrigger(
                text="Catalyst timeline passes without materialization",
                monitorable=True,
                check_frequency="monthly",
            ),
            DisconfirmingTrigger(
                text="New information invalidates the basis of our variant thesis",
                monitorable=True,
                check_frequency="weekly",
            ),
            DisconfirmingTrigger(
                text="Consensus revision momentum reverses sharply against our variant "
