...
def main():
    ...
    parser.add_argument("--backend", default="auto", choices=["auto", "sdk", "subprocess", "deepseek", "grok"],
                        help="LLM backend: auto (default, prefers DeepSeek), deepseek (DeepSeek, default API backend), "
                             "grok (xAI), sdk (Anthropic API), subprocess (claude CLI)")
    ...
    parser.add_argument("--grok-key", default=None, help="Grok API key (or set GROK_API_KEY / XAI_API_KEY env var)")
    parser.add_argument("--grok-model", default=None,
                        help="Grok model (default: GROK_MODEL env var, else grok-4)")
    parser.add_argument("--fast-agents", action="store_true",
                        help="Use cheaper model (deepseek-v4-flash) for light specialist agents, keep the flagship for synthesizer/editor")
