...
**✅ 验证闭环（2026-08-01，300502 全量 LLM run，29min / 23 调用 / $0.22）**：
- 盖章日志实证：`Customer concentration (L1): top5 72.34%（巨潮资讯年报 PDF《2025年年度报告》（300502，2026-04-24 披露））`，PDF 命中缓存零下载
- **效果代理指标达成**：旧 run（7 月）集中度相关命中 21 处，其中**定量**追问（"前五大占比是多少/是否超 30%"）≥6 条 open_questions + 3 位 agent 列为"关键缺失输入" + edge_durability 建立在"占比未知"上；新 run 定量追问 **0 条**，仅剩 2 条问客户**名字**与合作年限（年报匿名披露"第一名/第二名"，确实不含，合理保留）
- 报告正文直接引用且清洗白名单下存活：pros 块两处引 72.34%/22.97%/逐名占比，并做衍生计算"**前三家合计达 55.24%**"（22.97+16.64+15.63，算术闭合）+ 行业定位判断"集中度在通信设备行业中属于偏高水平"
- run 收尾健康：blocked by warn_accumulation_gate（5 agent 红旗，观察框架形态，历轮新易盛常态，与本改动无关）、thesis v2 持久化、Editor headline 条件化

**遗留/下一步**：
1. 解析覆盖仅验证 2025 年报模板；更早年份/金融业年报（无此披露）会干净降级 None，不阻断
2. 其余 4 只评测票的全量 LLM run 未重跑（PDF 缓存已种，随日常 launchd 扫描/下轮评测自然覆盖）
...
**主轨（冻结 thesis 尺 ×2 采样）3.45**：宁德 3.50 / 新易盛 3.25（三轮 3.0 首次松动）/ 茅台 3.50 / 北方华创 3.50（回吐 R6 峰值）/ 比亚迪 3.50——4/5 票两次采样完全一致，F1-F5 把产品标准化后 thesis 尺对"诚实观察框架"定价收敛在 3.5 整。**副轨（FRAMEWORK_RUBRIC_V1 ×2）3.65 基线首点**，扣分全落监控合约工程（判别器自毁/阈值三源打架）。轨迹 3.1→2.8→3.33→3.5→3.25→3.50→**3.45**。完整判读见 [GROK_REAUDIT_2026-07-12.md](GROK_REAUDIT_2026-07-12.md) Round 7 节。
宁德 F1-F5 实物：core_thesis ="股价在追问一个核心问题：21.1% OPM 是结构性堡垒还是锂价馈赠的周期性沙堡"+判别检验（H1 2026 OPM≥18%）+披露日历（半年报 49 天）——历轮最好的产品形态，主轨分数不涨 = 诚实折价实锤。
**结论：表达层榨干，L1 年报附注摄取（分部收入/客户集中度）为唯一主路。**
⚠ R7 首跑事故：Director 的 agent_depth 被 LLM 吐成 JSON 字符串 → orchestrator `.get()` 炸整条 run——BUG-Y25 的 dict 版，[coerce_dict](aegis/core/_coerce.py) 已修（d3ef1a3，+5 回归）；其余 LLM 解析边界的 dict 字段扫查已挂后台任务芯片。
...
**修复（commit f5228b6）**：R6-1 合成期 evidence gap 检查（引擎 B3 同源提前跑，gap 票进 strict 清洗 + variant_magnitude 确定性降级）；R6-2 merged_open_questions 双源合并（合约字段/形态判定/reason 计数三处同源）；R6-3 observation_framing_block（待解问题堆积票生成时条件化注入）+ 修 re-synth 漏传 open_questions 预存在缺口。测试 1648 绿（+10）。

**结果（logs/eval_round6 + grok_audits_round6，[eval_pipeline.sh](scripts/eval_pipeline.sh) 已参数化）**：北方华创 4.00(+1.0 vs R4) / 宁德 3.75 / 茅台 3.50 / 比亚迪 3.25 / 新易盛 3.00，**均分 3.50**。轨迹 3.1→2.8→3.33→3.5→3.25→**3.50**（后两轮均值测量）。

**剩余扣分墙 + 判决（详见 [GROK_REAUDIT_2026-07-12.md](GROK_REAUDIT_2026-07-12.md) Round 6 节）**：①软性目标价话术（"显著低估/修复空间巨大"绕过数字正则——表达层最后一发，半天量）；②新能源 pack GWh×ASP 套比亚迪整车主业（002371 NVDA 模板同款，applies_to 守卫需扩展）；③裸数字点估计漏网（宁德"237"无 ¥ 前缀）；④数据缺口每票通杀 + 新易盛卡死 3.0。**表达层收益递减确认，主路转 L1 年报附注摄取（客户集中度/分部收入）+ L2 独立预测层。**

## 📊 2026-07-13 凌晨 R5 评测轮（L4+L3 生效后首轮，每票 2 次采样）

**跑法**：[r5_pipeline.sh](scripts/r5_pipeline.sh)（5 票串行重生成，每票生成完立即后台起 AUDIT_RUNS=2 审计，与下票生成重叠；总墙钟 ~2h45m）。产物 `logs/eval_round5/` + `logs/grok_audits_round5/`。

**结果**：宁德 3.50(=) / 新易盛 3.00(−0.5) / 茅台 3.25(+0.25) / 北方华创 **3.75(+0.75，R5-1 驱动树守卫兑现)** / 比亚迪 2.75(−0.25)，**均分 3.25**（R4 3.2）。轨迹 3.1→2.8→3.33→3.5→3.2→3.25。**5 票全为 observation_framework**（4 票 evidence gap/downgraded 触发、1 票失配触发）。

**核心判读（详见 [GROK_REAUDIT_2026-07-12.md](GROK_REAUDIT_2026-07-12.md) Round 5 节）**：
- ✅ L4 标签层成功：5/5 判词裁定形态"匹配/正确产品决策"；L3 生效：两票极差 0、三票 0.5，均值可测量
- ❌ 分数不动的根因="内容没跟上形态"：①结论句未降级（比亚迪 core_thesis 首句仍是「135.52/+50.6%」——rule 7 只挂 mismatch 分支，本轮 4/5 票是带内票没被注入；editor 约束不覆盖合约字段）；②open_questions 双源不一致（reason 数 orchestrator 层 15-18 项 vs 合约字段取 synthesizer 输出为空，三票被点名"meta 与正文冲突"）
- **R6 清单**：R6-1 观察框架票合约级语气确定性再处理（审计者原话"应改成待验假设列表"）；R6-2 open_questions 双源统一（工程量最小、三票在漏分）；R6-3 注入面扩宽到待解问题堆积票；然后 L1 年报附注摄取
...
## 🔧 2026-07-13 BUG-Y25 dict 版 · 全 call_structured 解析边界扫查（任务芯片分支 funny-noyce-78e190，2026-07-31 已并入本支）
