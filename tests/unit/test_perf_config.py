...
D1: AGENT_MAX_PARALLEL=2 was a subprocess-CLI rate-limit guard applied to
    every backend — API backends (deepseek/sdk) now get their own cap
...
D3: DeepSeek cold-start max_tokens was flat (32768) for every
...
class TestTimeoutTiers:
    ...
    def test_api_env_overrides(self, monkeypatch):
        ...
        assert c.agent_watchdog_timeout_for("deepseek") == 600
...
class TestResolvedBackendKind:
    ...
    @staticmethod
    def _cfg(backend="auto", deepseek_key=""):
...
class TestHintCapabilityCheck:
    ...
    def test_deepseek_accepted(self, monkeypatch):
        monkeypatch.setenv("DEEPSEEK_API_KEY", "sk-test")
        from aegis.core.llm.deepseek_client import DeepSeekClient
