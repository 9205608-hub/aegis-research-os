...
class TestInsiderTradingCard:

    def test_card_renders_with_data(self):
        from aegis.core.reports.html_report import _build_insider_trading_card
        ...
        summary = InsiderSummary(
            ...
            transactions=[
                InsiderTransaction(
                    ...
                    transaction_date="2025-06-15", transaction_type="P",
                    shares=10000, price_per_share=150.0, total_value=1_500_000,
                    shares_owned_after=50000, is_direct=True,
                    filing_date="2025-06-17", accession_number="acc1",
                ),
            ],
            net_shares=10000,
            net_value=1_500_000,
        ...
        html = _build_insider_trading_card(summary)
        assert "Insider Trading Activity" in html
        assert "Bullish" in html
        assert "Tim Cook" in html
        assert "CEO" in html
        assert "Net Buying" in html

    def test_card_empty_when_no_data(self):
        from aegis.core.reports.html_report import _build_insider_trading_card

        assert _build_insider_trading_card(None) == ""

    def test_card_empty_when_no_transactions(self):
        from aegis.core.reports.html_report import _build_insider_trading_card

        summary = InsiderSummary(ticker="AAPL", period_months=12)
        assert _build_insider_trading_card(summary) == ""

    def test_card_shows_cluster_signal(self):
        from aegis.core.reports.html_report import _build_insider_trading_card
        ...
        summary = InsiderSummary(
            ...
            transactions=[
                InsiderTransaction(
                    filer_name="X", filer_title="Dir",
                    transaction_date="2025-06-15", transaction_type="P",
                    shares=100, price_per_share=100.0, total_value=10000,
                    shares_owned_after=1000, is_direct=True,
                    filing_date="2025-06-17", accession_number="a",
                ),
            ],
            cluster_detected=True,
            sentiment="bullish",
        ...
        html = _build_insider_trading_card(summary)
        assert "Cluster Detected" in html
