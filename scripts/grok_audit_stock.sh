...
# 双轨 A（2026-07-13 冻结，B 支 port）：观察框架票用 FRAMEWORK_RUBRIC_V1，
# 投资论点票用原 thesis 尺子（2026-07-11 冻结版）。FORCE_RUBRIC=thesis|framework
# 可强制（主 KPI 轨迹 = FORCE_RUBRIC=thesis；跨轨趋势参照亦用此开关）。
# 两把尺子文本自冻结日起不改。Grok 仲裁红线：框架尺分数只进副轨迹，
# 禁止与 thesis 尺历史均分接龙。
import os as _os
_force = _os.environ.get("FORCE_RUBRIC", "").strip().lower()
is_framework = (t.get('product_form') == 'observation_framework')
if _force == "thesis":
    is_framework = False
elif _force == "framework":
    is_framework = True

_artifact_label = ("条件化观察框架 + 监控合约（结构化 contract）" if is_framework
                   else "研究论点（结构化 thesis contract）")
body = [
    f"你是一名极其严格、坦诚、不吹捧的卖方研究质检 / 对抗性审稿人（grok-4.5）。",
    f"下面是 Aegis 智能投研系统对 A 股 **{name}（{code}）** 自动生成的{_artifact_label}。",
...
if is_framework:
    # FRAMEWORK_RUBRIC_V1（2026-07-13 冻结，不得修改）
    body += [
        "请**只做审计、不修改任何文件、不调用工具**，基于金融与会计常识做对抗性检查，输出**简体中文 markdown**，严格按下列 7 节：",
        "",
        "（本产出物自我声明为「条件化观察框架 + 监控合约」——请按观察框架的标准审计，而非按可下单投资论点的标准。观察框架的职责是：把'现价在定价什么'表述为可判别的竞争假设，并给出可执行的验证/证伪监控合约；它明示不提供建仓方向与幅度。）",
        "",
        "1. **数值一致性**：框架内数字是否自洽（增速/利润率/现金流/杠杆等算术是否闭合、有无自相矛盾）。逐条列出可疑数字与理由。",
        "2. **幻觉/无据数字**：有无凭空出现、无法从公开财报合理推出的具体数字。",
        "3. **中心问题与竞争假设质量**：'现价在定价什么'的中心问题是否成立且重要？两个竞争假设是否互斥、可判别、覆盖主要可能性？还是变相夹带单边结论？",
        "4. **判别性检验与监控合约可执行性**：判别检验/监控点/阈值/证伪触发是否可观测、阈值单一、检查频率与指标期限匹配？到复盘日能否干净打分？",
        "5. **形态判定合理性**：该票被判为观察框架的理由（数据缺口/估值失配/发布门）是否属实？形态声明与正文语气是否一致（有无暗度陈仓的单边结论）？",
        "6. **数据闭合路径**：待解问题与监控计划是否覆盖'升级为投资论点'所需的关键数据？获取路径与时间表是否具体可行？",
        "7. **总评**：一句话总评 + 按严重度排序的**最严重 3 个问题** + 一个 **0–10 的『作为观察框架/监控合约用于真实投研流程的可信度』打分**（并给一句打分依据）。",
        "",
