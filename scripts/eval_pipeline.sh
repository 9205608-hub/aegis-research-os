...
# 用法: bash scripts/eval_pipeline.sh <轮号>    # 如 6
# 产物: logs/eval_round{N}/{code}.log + logs/grok_audits_round{N}/
# 注意 macOS 自带 bash 3.2 无 declare -A，用平行数组。
set -u
cd "$(dirname "$0")/.." || exit 1

N="${1:?need round number, e.g. 6}"
ROUND="eval_round${N}"
...
echo "[$(date +%H:%M:%S)] R${N} pipeline start" >> "$PROGRESS"
AUDIT_PIDS=""
